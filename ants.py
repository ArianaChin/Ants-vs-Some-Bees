...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if type(insect) == QueenAnt and insect.is_true:
                return
            # END Problem 13
    ...
    def nearest_bee(self, hive):
        ...
        if self.max_range:
            for j in range(self.max_range):
                if p.entrance!= hive and random_or_none(p.bees)==None:
                    p = p.entrance
                else:
                    break
            ...
            if not p:
                return
            while p.entrance != hive and random_or_none(p.bees)==None:
                p = p.entrance
            return random_or_none(p.bees)
        # END Problem 3 and 4
    ...
    def action(self, colony):
        ...
        def helper(place):
            if place.ant:
                ...
                if place.ant.is_container and place.ant.contained_ant and not place.ant.contained_ant.doubled:
...
def make_slow(action, bee):
    ...
    def new_action(colony):
        if bee.effected and bee.duration[bee.effected] > 0:
            if colony.time%2 == 0:
                bee.effected -= 1
                bee.alt_action[bee.effected + 1](colony)
                bee.effected += 1
            bee.duration[bee.effected] -= 1
            if bee.duration[bee.effected]<= 0 and len(bee.duration)>bee.effected and bee.duration[bee.effected+1]:
                bee.alt_action[bee.effected] = bee.alt_action[1]
            elif bee.duration[bee.effected] <= 0:
                bee.effected -= 1
        else:
            bee.alt_action[1](colony)
    return new_action
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    if duration > 0:
        bee.effected += 1
        bee.alt_action[bee.effected] = bee.action
        bee.duration[bee.effected] = duration
        bee.action = effect
    #print("Duration: " + str(bee.duration))
    # END Problem EC
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            apply_effect(make_slow(target.action,target), target, 3)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target and not target.is_scared:
            apply_effect(make_scare(target.action,target), target, 2)
